...
namespace atakafe_api.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<ActionResult<Order>> Get(int id, int? staffId)
        {
            ...
            if (staff != null && !staff.HasFullAccess && !isOwner && ret.StaffId != staff.Id)
            {
                return null;
            }
            return ret;
        ...
        public async Task<ActionResult<bool>> Remove(int id, int? staffId)
        {
            ...
            {
                var oldOrderDate = oldOrder.CreatedAt.HasValue ? oldOrder.CreatedAt.Value : DateTime.UtcNow;
                if (oldOrderDate.AddHours(staff.HourLimit) < DateTime.UtcNow && !staff.CanUpdateDeleteOrder)
                {
                    return null;
                }
        ...
        private async Task CreateTransactionsAsync(Order order, string lang)
        {
            ...
            newTrade.CreatedAt = order.CreatedAt.HasValue ? order.CreatedAt.Value : DateTime.UtcNow;
