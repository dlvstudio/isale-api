...
namespace atakafe_api.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("List")]
        public async Task<IEnumerable<Product>> List(int? staffId, int? storeId, bool? isMaterial)
        {
            ...
            var key = (storeId.HasValue ? storeId.Value : 0) + "-" + (isMaterial.HasValue ? isMaterial.Value : false);
            ...
            var products = await _productService.GetProducts(userId, storeId.HasValue ? storeId.Value : 0, isMaterial.HasValue ? isMaterial.Value : false);
        ...
        [HttpGet]
        [Route("ListExpiries")]
        public async Task<IEnumerable<Product>> ListExpiries(int? staffId, int? storeId, bool? isMaterial)
        {
            var userId = User.GetUserId();
            var isShopOwner = true;
            if (staffId.HasValue && staffId.Value > 0)
            ...
            var post = await _productService.GetProductsWithExpiry(userId, storeId.HasValue ? storeId.Value : 0, isMaterial.HasValue ? isMaterial.Value : false);
            return post;
        }
        ...
        public async Task<IEnumerable<Product>> ListQuantities(int? staffId, int? storeId, bool? isMaterial)
        {
            var userId = User.GetUserId();
            var isShopOwner = true;
            if (staffId.HasValue && staffId.Value > 0)
            ...
            var post = await _productService.GetProductsWithQuantity(userId, storeId.HasValue ? storeId.Value : 0, isMaterial.HasValue ? isMaterial.Value : false);
            return post;
        }
        ...
        public async Task<ActionResult<int>> SaveProduct(SaveProductViewModel productViewModel)
        {
            ...
            product.Count = productViewModel.Count.HasValue ? productViewModel.Count.Value : 0;
