...
namespace atakafe_api.Controllers
{
    ...
    public class ExcelController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> DownloadSalesReportExcel(GetOrdersViewModel model)
        {
            ...
            return await _excelService.CreateSalesReportFile(model.ReportType.HasValue ? model.ReportType.Value : 0, model.Lang, userId, dateFrom, dateTo, storeId, staffId);
        }
        ...
        public async Task<ActionResult<OrderReportSummaryViewModel>> SalesReport(GetOrdersViewModel model)
        {
            ...
            if (reportType == 0)
            {
                return await _excelService.BuildSalesReportByOrder(model.Lang, userId, dateFrom, dateTo, storeId, staffId);
            }
            if (reportType == 1)
            {
                return await _excelService.BuildSalesReportByProduct(model.Lang, userId, dateFrom, dateTo, storeId, staffId);
            }
            if (reportType == 2)
            {
                return await _excelService.BuildSalesReportByCustomerAndProduct(model.Lang, userId, dateFrom, dateTo, storeId, staffId);
            }
            if (reportType == 3)
            {
                return await _excelService.BuildSalesReportByStaff(model.Lang, userId, dateFrom, dateTo, storeId, staffId);
            }
            return null;
        }
        ...
        public async Task<ActionResult<object>> ProductReport(GetOrdersViewModel model)
        {
            ...
            if (reportType == 0)
            {
                return await _excelService.BuildProductReportSummary(model.Lang, userId, dateFrom, dateTo, storeId);
            }
            if (reportType == 1)
            {
        ...
        public ActionResult<dynamic> PdfConvert(ConvertHtmlModel htmlModel)
        {
            ...
            {
                Byte[] res = null;
                var pdf = TheArtOfDev.HtmlRenderer.PdfSharp.PdfGenerator.GeneratePdf(htmlModel.Html, htmlModel.Size == 5 ? PdfSharp.PageSize.A4 : PdfSharp.PageSize.Letter, 5);
                pdf.Save(ms);
                res = ms.ToArray();
        ...
        {
            var pdf = TheArtOfDev.HtmlRenderer.PdfSharp.PdfGenerator.GeneratePdf(htmlModel.Html, htmlModel.Size == 5 || htmlModel.Size == 6 ? PdfSharp.PageSize.A5 : PdfSharp.PageSize.Letter, 20);
