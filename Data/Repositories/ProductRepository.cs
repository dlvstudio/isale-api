...
public class ProductRepository : IProductRepository
{
    ...
    public async Task<IEnumerable<ProductNote>> GetNextProductNotesById(DateTime createdAt, int productId, string userId, int storeId)
    {
        using (var db = AppDb)
        {
            string query = @"SELECT
            ...
            await db.Connection.OpenAsync();
            var result = await db.Connection.QueryAsync<ProductNote>(query, new { CreatedAt = createdAt, UserId = userId, ProductId = productId, StoreId = storeId });
            return result;
        }
    }

    public async Task<ProductNote> GetPreviousProductNoteById(DateTime createdAt, int productId, string userId, int storeId)
    {
        using (var db = AppDb)
        {
            string query = @"SELECT
