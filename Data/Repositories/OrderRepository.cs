...
public class OrderRepository: IOrderRepository {
    ...
    public async Task<IEnumerable<Order>> GetOrders(string userId, DateTime dateFrom, DateTime dateTo, int contactId, int staffId, int storeId, int? status) {
        ...
        using (var db = AppDb)
        {
            ...
            var result = await db.Connection.QueryAsync<Order>(query, new { UserId = userId, DateFrom = dateFromOnlyDate, DateTo = dateToAddOne, ContactId = contactId, StaffId = staffId, StoreId = storeId, Status = (status.HasValue ? status.Value : -1) });
    ...
    public async Task<int> SaveOrder(Order order) {
        ...
        using (var db = AppDb)
        {
            ...
            if (order.Id > 0) {
                ...
                isInsert = true;
                order.CreatedAt = order.CreatedAt.HasValue ? order.CreatedAt.Value : DateTime.UtcNow;
